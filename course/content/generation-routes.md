...
Статус: учебный кандидат v6 / выбор модели и режима отделён от компиляции управляемого сегмента.
...
## 06.11. Следующий Шаг: Собрать Сегмент

После этой главы у вас есть решение:

```text
какой тип модели нужен;
какой режим подходит задаче;
какие материалы интерфейс действительно принимает;
какие сведения нужно проверить заново.
```

Теперь переходите к `06A_VIDEO_PROMPTS_AND_CONTROLLED_SEGMENTS.md`. Там выбранный маршрут превращается в карточку одного клипа, чистый английский видеопромпт, точное финальное состояние и проверку реального результата.

📌 Модель и режим — это способ выполнить задачу. Они не заменяют тайминг, карту кадров и принятые входные кадры.
