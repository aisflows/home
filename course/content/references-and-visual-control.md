...
Статус: учебный кандидат v11 / Reference System отделена от геометрии сцены, Shot Plan и Frame Pack.
...
## 04.13. Переход К Пространству И Карте Кадров

После этой главы у вас есть **Reference System**: понятно, какие материалы отвечают за лицо, объект, локацию, свет, материал и стиль.

Но референс локации ещё не определяет автоматически:

- где стоят герои в конкретной сцене;
- где проходит ось действия;
- что находится слева и справа для зрителя;
- какой ракурс нужен в каждый момент;
- какие статичные кадры надо сгенерировать.

Поэтому следующий порядок такой:

```text
04A — геометрия сцены и пространственная непрерывность;
04B — Shot Plan / Storyboard Map;
05 — короткая граница между image prompt и video prompt;
05A — производство и проверка Frame Pack.
```

📌 **Итог главы:** референсы получили роли. Они больше не являются кучей красивых картинок, которые одновременно спорят за лицо, свет и композицию.
